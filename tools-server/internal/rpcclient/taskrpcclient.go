...
func NewTaskRpcClient(etcdutil *etcd.ETCDUtil) *ToolRpcClient {
	server := &ToolRpcClient{
		Servers: make(map[string]*rpcpool.RpcTaskPool),
	}
	addrs := etcdutil.DiscoverServices("ToolServer", server.UpdateServer)
	fmt.Println(addrs)
	return server
}
func (client *ToolRpcClient) Translation(ctx context.Context, param *toolservice.TranslationRequest, addr string) (*toolservice.TranslationReply, error) {
	...
	toolclient := toolservice.NewToolServiceClient(conn)
	result, err := toolclient.Translation(ctx, param)
	return result, err
}
...
*/
func (client *ToolRpcClient) UpdateServer(addrs []string) {
	fmt.Println("服务发生变动")
...
func (client *ToolRpcClient) Close() {
